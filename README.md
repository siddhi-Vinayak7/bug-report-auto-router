...
Multi-label bug triage system featuring an **LLM-primary decision architecture** (Groq `llama-3.3-70b-versatile`) backed by a TF-IDF + Logistic Regression classifier for context and fallback, with explainability, department routing, and active learning correction logging.
...
2. **LLM Primary Decision**: The original report text and the classifier's predictions (framed as a reference signal) are sent to the Groq API (`llama-3.3-70b-versatile`). If the LLM returns a valid module and severity, its response becomes the final decision (`decision_source: "llm"`).
...
## Features
- **LLM-Primary Module & Severity Prediction**: Evaluates module (`Auth`, `Chat`, `Tasks`, `Profile`, `Payments`, `Other`) and severity (`Critical`, `Major`, `Minor`) using Groq `llama-3.3-70b-versatile` with classifier context.
