"""
severity_classifier_svm.py
--------------------------
SVM-based severity classifier. Mirrors severity_classifier.py but uses
LinearSVC instead of LogisticRegression.

Intended only for comparison -- does NOT overwrite severity_model.pkl or
modify any existing file. Saves its own model to severity_model_svm.pkl.
"""
...
MODEL_PATH = BASE_DIR / "severity_model_svm.pkl"
...
def tune_severity_svm(train_csv_path: Path = TRAIN_DATA_PATH) -> float:
    """
    ...
    print("=" * 60)
    print("SEVERITY CLASSIFIER -- SVM (LinearSVC) -- CV RESULTS")
    ...
    print(f"\n  >>> Best C for Severity SVM: {best_c}  (CV accuracy: {best_mean:.4f})")
    print("=" * 60)
    return best_c
...
def train_severity_svm(
    ...
    set with the chosen C, and saves it to severity_model_svm.pkl.
    """
    best_c = tune_severity_svm(train_csv_path)
    ...
    print(f"\nSeverity SVM model saved to {model_save_path}")
    return pipeline
...
def load_severity_svm_model(model_path: Path = MODEL_PATH) -> Pipeline:
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            ...
            "Run train_severity_svm() first."
        )
    _cached_pipeline = joblib.load(model_path)
    return _cached_pipeline


def predict_severity_svm(text: str) -> str:
    """Returns the predicted severity label using the SVM model."""
    pipeline = load_severity_svm_model()
